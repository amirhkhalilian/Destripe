...
% img = double(imread('Megaphragama122017-1 8x8x8nm-cropped0213.tif'));
[m,n] = size(img);
%% fft calculation
im_fft = fft2(img);
...
for ring_radius = ring_radius_init:ring_halfwidth:ring_radius_end
...
cutoff = mean(abs(fft_in_ring))+2*std(abs(fft_in_ring));
...
for i =1:length(outlier_ind)/2
	...
	if allout==1
		if ((~isempty(find(prev_pos_x==pos_x(outlier_ind(i)))))&&(~isempty(find(prev_pos_y==pos_y(outlier_ind(i))))))
			img_fft_shift(pos_x(outlier_ind(i)),pos_y(outlier_ind(i))) = (img_fft_shift(pos_x(outlier_ind(i)),pos_y(outlier_ind(i)))+sample)/2;
			img_fft_shift(pos_x(end-outlier_ind(i)+1),pos_y(end-outlier_ind(i)+1)) = (img_fft_shift(pos_x(end-outlier_ind(i)+1),pos_y(end-outlier_ind(i)+1))+conj(sample))/2;
		else
...
prev_outlier_ind = outlier_ind;
prev_mean_real = mean_real;
prev_std_real = std_real;
prev_mean_imag = mean_imag;
prev_std_imag = std_imag;
prev_pos_x = pos_x;
prev_pos_y = pos_y;
